...
import { memoryService, StudentMemory, DAILY_CAP } from "../services/memoryService";
 ...
 */
export function Dashboard({ onPickTopic }: DashboardProps) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {insights.map((it, i) => (
                <div key={i} className="bg-white border border-slate-200 rounded-xl p-5">
                  ...
                  <p className="text-sm text-slate-700 leading-relaxed">"{it.insight}"</p>
                </div>
              ))}
            </div>
          </section>
