 ...
 */
export function CapDialog({ resetAt, cap, onClose, onUpgrade }: CapDialogProps) {
        ...
        <div className="px-7 py-7">
          ...
          <div className="bg-brand-accent/5 border border-brand-accent/10 rounded-xl p-4 mb-5 flex items-start gap-3">
            ...
            <div className="text-xs text-brand-accent/90 leading-relaxed">
              <b>Sambung streak anda esok.</b> Atau naik taraf ke Premium Pro untuk kuota tanpa batasan dan sesi belajar tanpa henti!
            </div>
          </div>
          ...
          {onUpgrade && (
            <button
              onClick={onUpgrade}
              className="w-full mb-2.5 py-3 rounded-xl bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-600 hover:to-yellow-650 font-semibold text-slate-950 text-xs tracking-wide shadow-md hover:shadow-lg transition active:scale-[0.98] flex items-center justify-center gap-2"
            >
              👑 Langgan Premium Cikgu Pro
            </button>
          )}

          <button
            onClick={onClose}
            className="w-full py-2.5 rounded-xl text-slate-500 hover:bg-slate-100 font-medium text-xs transition"
          >
            Nanti Dulu, Jumpa Esok
          </button>
        </div>
      </motion.div>
    </motion.div>
