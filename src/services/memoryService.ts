...
export const DAILY_CAP_PREMIUM = 80;
...
export const memoryService = {
   ...
   */
  async canSend(userId: string): Promise<{ ok: boolean; remaining: number; resetAt: number; limit: number }> {
    const userEmail = auth.currentUser?.email;
    const mem = await this.getMemory(userId);
    const isSub = !!mem.isSubscriber;

    if (isAdmin(userEmail)) {
      return { ok: true, remaining: 999, resetAt: nextResetAt(), limit: 999 };
    }

    const currentCap = isSub ? DAILY_CAP_PREMIUM : DAILY_CAP;
    const used = mem.dailyMessages ?? 0;
    return {
      ok: used < currentCap,
      remaining: Math.max(0, currentCap - used),
