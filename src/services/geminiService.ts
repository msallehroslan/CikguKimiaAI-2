  ...
  async sendMessageStream(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const isHtml = serverError.trim().startsWith("<") || 
                       serverError.toLowerCase().includes("<html") || 
                       serverError.toLowerCase().includes("<!doctype");
        const cleanErr = isHtml || !serverError.trim() ? `HTTP ${response.status}` : serverError.trim();
